...
`kaggle/m5-forecast.ipynb` runs the whole method on Kaggle and writes `submission.csv`; see
`kaggle/README.md` for the steps. It embeds the pipeline modules from `src/` (generated by
`tools/build_kaggle_notebook.py`). Checked locally with the full-size models: the submission it
writes scores 0.5866 on the private window, the same as the reported result, and a quick-mode
run exercises every cell end to end.
